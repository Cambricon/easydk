#ifndef __CN_SENSOR_327_OBJ_H__
#define __CN_SENSOR_327_OBJ_H__
...
#define cnSens327Print(format,args...) \
do{ \
    cnLog(CN_DBG_INFO,  CN_ID_ISP, "/%s/%s/%d imx327 -- "format, __FILE__,__func__,__LINE__, ##args);\
} while (0)

// note high bit 32 is refresh flag
typedef struct _cn_imx327_dynamic_parm
{
...
typedef struct _sensor_cxt_t{
    ...
    cn_imx327_dynamic_parm_t stDynamicParm;
    cnSensorCfg_s stSensorCfg;
    cnSensorBusInfo_s stBusInfo;
    cn_imx327_ae_init_parm_t stAeInitParam;
    cnSensorCalibrations_s *pstCalibration;
}cnSensorCxtImx327;

#define M_MULT_SENSOR_327
